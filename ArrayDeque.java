...
public class ArrayDeque<T> {
    ...
    public void addFirst(T data) {
        if (data == null) {
            throw new IllegalArgumentException("Cannot insert null data to the deque.");
        }
        if (size == backingArray.length) {
            T[] resizedArray = (T[]) new Object[backingArray.length * 2];
            resizedArray[0] = data;
            for (int i = 0; i < backingArray.length; i++) {
                resizedArray[i + 1] = backingArray[(front + i) % backingArray.length];
            }
            front = 0;
    ...
    public T removeFirst() {
        if (size == 0) {
            throw new NoSuchElementException("Cannot remove first element from an empty deque.");
        }
    ...
    public T removeLast() {
        if (size == 0) {
            throw new NoSuchElementException("Cannot remove last element from an empty deque.");
        }
        int index = (front + size - 1) % backingArray.length;
        T removeItem = backingArray[index];
    ...
    public T getFirst() {
        if (size == 0) {
            throw new NoSuchElementException("Cannot retrieve first element from an empty deque.");
        }
    ...
    public T getLast() {
        if (size == 0) {
            throw new NoSuchElementException("Cannot retrieve last element from an empty deque.");
        }
